/// get_edition_summary_v2.cdc — Return Edition metadata as a flat
/// `{String: AnyStruct}` dictionary with explicit primitive types.
///
/// Replaces the original artdrop/cdc/get_edition_summary.cdc that returns
/// `ArtDropCore.EditionSummary?` (the contract struct). Two contract-side
/// fields the Go handler used to read don't match what the contract
/// actually exposes:
///
