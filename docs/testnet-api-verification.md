...
Deployed to Quave as v53 (commit `b9a2db0`). **Re-verified live against the deployed API** (not just direct CLI): `POST /v1/accounts/0x0daaba937562c85f/transfer?sync=true {"certificateId":3,"to":"0x0a478c507cc8ea88"}` → `HTTP 201`, `ArtDropCore.CertificateTransferred` event emitted with `viaProtocolTransfer` — tx `9bcf04643998ac1db72d1d8eba0425c24fbebd1d11b5ccf9d4252a495b27e693`. Issue closed — `/transfer` fully works end-to-end via the live REST API.
